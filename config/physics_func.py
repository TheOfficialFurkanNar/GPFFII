...
class ModifiedDispersionRelation:
    ...
    def _planck_mass(self):
        ...
        hbar = PLANCK_CONSTANT.magnitude / (2 * math.pi) if hasattr(PLANCK_CONSTANT,
                                                                    'magnitude') else PLANCK_CONSTANT / (2 * math.pi)
        c = SPEED_OF_LIGHT.magnitude if hasattr(SPEED_OF_LIGHT, 'magnitude') else SPEED_OF_LIGHT
        G = GRAVITATIONAL_CONSTANT.magnitude if hasattr(GRAVITATIONAL_CONSTANT, 'magnitude') else GRAVITATIONAL_CONSTANT
    ...
    def mode_energy(self, p: float, mode: GPFFIIMode) -> float:
        ...
        hbar = PLANCK_CONSTANT.magnitude / (2 * math.pi) if hasattr(PLANCK_CONSTANT,
                                                                    'magnitude') else PLANCK_CONSTANT / (2 * math.pi)
        M_val = self.M.magnitude if hasattr(self.M, 'magnitude') else self.M
...
class ObservedFlux:
    ...
    def observed_flux(self, luminosity: float, distance: float, modes: list,
        ...
        if p is None:
            ...
            h = PLANCK_CONSTANT.magnitude if hasattr(PLANCK_CONSTANT, 'magnitude') else PLANCK_CONSTANT
            c = SPEED_OF_LIGHT.magnitude if hasattr(SPEED_OF_LIGHT, 'magnitude') else SPEED_OF_LIGHT
            p = h / 100e-9  # kg·m/s
...
class ClassicalFormulas:
    ...
    def wien_peak_wavelength(temperature: float) -> float:
        ...
        """
        b = WIEN_DISPLACEMENT.magnitude if hasattr(WIEN_DISPLACEMENT, 'magnitude') else WIEN_DISPLACEMENT
    ...
    def stefan_boltzmann_luminosity(temperature: float, radius: float) -> float:
        ...
        float - Luminosity (W)
        """
        sigma = STEFAN_BOLTZMANN_CONSTANT.magnitude if hasattr(STEFAN_BOLTZMANN_CONSTANT,
                                                               'magnitude') else STEFAN_BOLTZMANN_CONSTANT
