...
from config.physics_func import GPFFIIMode, ModifiedDispersionRelation, ObservedFlux, ClassicalFormulas, validate_mode_normalization
...
class GPFFIIVisualizer:
    ...
    def _au(self):
        return AU.magnitude if hasattr(AU, 'magnitude') else AU

    def _pc(self):
        return PC.magnitude if hasattr(PC, 'magnitude') else PC

    def _const(self, c):
        return c.magnitude if hasattr(c, 'magnitude') else c
    ...
    def _build_mode_list(self):
        ...
        # Build modes based on selected curvature sector
        modes = []
        
        # Add the selected curvature sector mode with full weight
        modes.append(GPFFIIMode(a=self.curvature_sector, i=0, w=1.0, lambda_i=0.0, kappa=0.0))
        
        # Add other curvature sectors with zero weight for completeness
        for a in [-1, 0, 1]:
            if a != self.curvature_sector:
                modes.append(GPFFIIMode(a=a, i=0, w=0.0, lambda_i=0.0, kappa=0.0))
        
        # Add speed branch modes if LIV coupling is active
        if abs(self.current_mode.lambda_i) > 0.01:
            modes += [
                GPFFIIMode(a=self.curvature_sector, i=-1, w=0.0, lambda_i=self.lambda_sub, kappa=0.0),
                GPFFIIMode(a=self.curvature_sector, i=1,  w=0.0, lambda_i=self.lambda_sup, kappa=0.0),
            ]
        
        # Validate mode parameters and normalization
        for mode in modes:
            mode.validate()
    ...
    def _calculate_flux_values(self, wavelengths, modes, h, c, k_B):
        ...
        for idx, wl in enumerate(wavelengths):
            ...
            if idx % 10 == 0:
                self.logger.info("flux_calculation_progress", {
